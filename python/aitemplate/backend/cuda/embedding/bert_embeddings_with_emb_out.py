"""
modified version of bert_embeddings kernel codegen for CUDA.
pretty much the same as original bert_embeddings.py but this also outputs embeddings prior to layer norm as well
"""
...
__global__ void bert_embeddings_with_emb_kernel(
    uint4* output,
    uint4* emb_output,
    INDEX_T* input_ids,
...
#pragma unroll
  ...
  // pure embedding calculation is done, so save it on emb_output
  if (tid < embedding_dim_div_n) {
      emb_output = emb_output + bid * embedding_dim_div_n;
      emb_output[tid] = embedding;
  }
...
template <typename ElemT, typename INDEX_T>
void bert_embeddings_with_emb_launcher(
    ElemT* output,
    ElemT* emb_output,
    INDEX_T* input_ids,
    INDEX_T* token_type_ids,
  ...
  bert_embeddings_with_emb_kernel<{{elem_input_type}}, INDEX_T><<<grid, block, 0, stream>>>(
      reinterpret_cast<uint4*>(output),
      reinterpret_cast<uint4*>(emb_output),
      input_ids,
      token_type_ids,
      position_ids,
...
{{func_signature}}
{
    bert_embeddings_with_emb_launcher<{{elem_input_type}}, {{index_type}}>(
      static_cast<{{elem_input_type}}*>(output),
      static_cast<{{elem_input_type}}*>(emb_output),
      input_ids,
      token_type_ids,
      position_ids,
...
@registry.reg("cuda.bert_embeddings_with_emb.gen_function")
def bert_embeddings_with_emb_gen_function(func_attrs: Dict[str, Any]) -> str:
...
@registry.reg("cuda.bert_embeddings_with_emb.func_decl")
def bert_embeddings_with_emb_gen_function_decl(func_attrs: Dict[str, Any]) -> str:
    dtype = python_int_dtype_to_c_dtype(func_attrs["inputs"][0]._attrs["dtype"])
...
@registry.reg("cuda.bert_embeddings_with_emb.func_call")
def bert_embeddings_with_emb_gen_function_call(func_attrs: Dict[str, Any], indent="  ") -> str:
    (
        input_ids,
        token_type_ids,
        position_ids,
    ...
    emb_output_str = func_attrs["outputs"][1]._attrs["name"]
